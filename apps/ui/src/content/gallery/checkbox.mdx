...
Match the container and state-layer sizes to turn the checkbox into a compact toggle button. The native checkbox still owns keyboard interaction, form state and its accessible checked state.

<GalleryGrid>

```html tag=MdxCodeBlock,label=Notification toggle
<style>
  .notification-toggle {
...
<c2-checkbox class="notification-toggle" aria-label="Toggle notifications">
