...
// One entry per generated icon so consumers can import `@c2n/feather-icons/icons/<name>.js`
...
export default defineConfig({
  build: {
    lib: {
      entry: {
        index: 'src/index.ts',
        'feather-icon': 'src/feather-icon.ts',
  ...
  plugins: [
    VitePluginCustomElementsManifest({
      files: ['src/feather-icon.ts', 'src/icons/*.ts'],
      lit: true,
