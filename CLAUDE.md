...
Release: `npm run build && npm run build:tools` then `npx lerna publish patch --no-private --exact --yes` (the skill package's `version` lifecycle script re-syncs `.claude-plugin/plugin.json` and the `@c2n/mcp@<version>` pin in its `.mcp.json`).
