...
`checked` reflects the state and `change` fires on every user toggle; call `toggle()` (optionally with `true` or `false`) to flip it from code and fire the same event, or set `checked` to change it silently. Give it a `name` and its `value` (`on` by default) appears in `FormData(form)` while checked, just like a native checkbox. It also supports `required`, fieldset disabling, `form.reset()`, constraint validation and custom validity messages.
