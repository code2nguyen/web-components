...
Rules that hold for every shape: prefer child variables on the host element or a class; use `::part()` only when the component API documents the part and variables cannot express the change; register variants under your own prefix (`app-*`, `site-*`), never `c2-*`; one file per variant in one directory (`src/components/ui/` here).
