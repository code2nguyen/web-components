...
Style assigned slot nodes with classes. Public `dropzone`, `drop-icon`, `prompt`, `hint`, `action`, `empty`, `errors`, and `list` parts expose the upload-owned interactive, fallback, validation, and queue regions.
