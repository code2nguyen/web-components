...
Choose where it opens with `placement`: `top`, `right`, `bottom` or `left`, optionally aligned with `-start` or `-end`. `offset` sets the gap from the trigger and `cross-offset` shifts the menu sideways along the trigger. All offsets are pixel numbers, and floating-ui can still flip the menu when the preferred side has no room.
...
<div data-label="Icon trigger and placement">
  ...
  <c2-menu client:only="lit" aria-label="Upwards" placement="top-start" offset="12" cross-offset="8">
...
<c2-menu anchor="row-7" placement="right-start" offset="12" cross-offset="4">
  <c2-menu-item value="edit">Edit</c2-menu-item>
</c2-menu>
```
