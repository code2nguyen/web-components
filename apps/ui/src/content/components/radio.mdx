...
`c2-radio` is one option: a native `<input type="radio">` with its label beside it and an optional `description` slot underneath. `c2-radio-group` turns several of them into one form-associated control with one `value` and one `name`, disables them together, and restores the native keyboard model across the shadow boundary: Arrow keys move and select, and only the selected option is a tab stop. Radios can sit anywhere inside the group, including in wrapper elements. Standalone radios sharing a `name` in the same root still uncheck each other and participate in forms individually, but only the group provides arrow-key navigation and a single `change` event with the new value.
...
Give a group `name` and its selected value appears once in `FormData(form)`; standalone radios submit their own value only while checked. `required`, fieldset disabling, `form.reset()`, `checkValidity()`, `reportValidity()` and `setCustomValidity()` follow the native form contract. The `label` slot names the group for assistive technology (`aria-label` works too); give a standalone radio without visible text an `aria-label`.
