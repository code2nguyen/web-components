...
export class Avatar extends LitElement {
  ...
  @property({ type: Number }) initialCount = 1

  get initial(): string {
    const initials = this.name.split(' ').map((n) => n.charAt(0))
    return initials.slice(0, this.initialCount).join('').toUpperCase()
  }

  override render() {
    return html` <div class="c2-avatar"><slot>${this.initial}</slot></div> `
  }
}
